...
class Gracl
    ...
    def initialize
        ...
        self.logfile        = File.expand_path("~/.gracl/logs/gracl.log")
    end
    ...
    def install_ssh_config
        logger.info { "generating new ~/.ssh/authorized_keys" }
        if !File.directory?(File.expand_path("~/.ssh"))
            logger.debug { "mkdir ~/.ssh" }
    ...
    def install_hooks
        logger.info { "installing repository hooks" }
